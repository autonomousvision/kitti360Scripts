...
args.groundTruthListFile = '/Users/yiyi/Documents/AVG/KITTI360/KITTI-360/data_windows/2013_05_28_drive_test_p1_nvs_input_frames.txt'
...
def generateMatrix(args):
    ...
    for label in labels:
        ...
        # we use label.trainId instead of label.id as used in Cityscapes
...
def evaluatePair(predictionImgFileName, groundTruthImgFileName, groundTruthConfFileName, confMatrix, instanceStats, perImageStats, args):
    ...
    if args.evalInstLevelScore:
        ...
        instList = np.unique(instanceNp[instanceNp%1000 > 0])
        for instId in instList:
            ...
            tp = np.count_nonzero( predictionNp[mask] == trainId )
            fn = instSize - tp
...
def main():
    ...
    if True:
        ...
        args.exportFile = os.path.join(os.path.abspath("results"), argv[0], "resultPixelLevelSemanticLabeling.json")
